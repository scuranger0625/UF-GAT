...
# =========================
# 7. 推論與指標/繪圖（高效版，避免 roc_curve 排序地獄）
# =========================
...
# --- 先算 AUC（這個會做排序，但只做一次，且比 roc_curve 輕很多）
auc_val = roc_auc_score(y_true_np, pred)

# --- 向量化找最佳門檻（以全資料 F1 最大為準；你也可以改只看 class 1 的 F1）
...
y = y_true_np[None, :].astype(np.uint8)                 # [1,N]
...
f1_arr        = np.where(precision_arr+recall_arr>0,
                         2*precision_arr*recall_arr/(precision_arr+recall_arr), 0.0)
...
# --- 向量化輸出 per-class sweep（0/1 類別的 Precision/Recall/F1）
# 類別1（正類）已經有 TP,FP,FN,TN
...
# 類別0視角：把0當正類 => TP0=TN, FP0=FN, FN0=FP
...
print("每個門檻下各類別指標已存檔：threshold_sweep_per_class.csv")

# --- 近似 ROC（分箱法，避免對 950 萬點排序）
